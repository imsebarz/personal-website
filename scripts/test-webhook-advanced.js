...
async function runLoadTest(requestCount) {
  log(`🚀 Ejecutando test de carga con ${requestCount} requests...`, colors.cyan);
  ...
  try {
    ...
    if (response.status === 200) {
      const loadTest = response.data.loadTest;
      
      log(`📊 Resultados del Test de Carga:`, colors.bold);
      log(`   Total de requests: ${loadTest.totalRequests}`);
      log(`   Exitosos: ${loadTest.successfulRequests}`, colors.green);
      log(`   Fallidos: ${loadTest.failedRequests}`, colors.red);
      log(`   Tiempo total: ${loadTest.totalTime}ms`);
      log(`   Tiempo promedio: ${loadTest.averageTime}ms`);
      log(`   Requests por segundo: ${loadTest.requestsPerSecond}`);
      
      if (loadTest.failedRequests > 0) {
        log(`\n❌ Requests fallidos:`, colors.red);
        loadTest.results
          .filter(r => !r.success)
          .forEach(result => {
            log(`   - Request #${result.requestNumber}: ${result.error || 'Error desconocido'}`);
          });
      }
    } else {
      log(`❌ Error en test de carga: ${response.status}`, colors.red);
    }
  } catch (error) {
    log(`❌ Error en test de carga: ${error.message}`, colors.red);
  }
}
